...
both, layered — a **Target** (weights, rules, margin policy) and a **Path** (the ordered Moves
...
**Path only** — a checklist of accepted moves, no end state. Rejected: changing strategy means
...
reports all fall out of Moves rather than being three separate features.
...
A Move points at a **destination**, not necessarily a security. Paying down margin and holding cash
compete with buying for every dollar freed — early in a transition, retiring debt at the margin
rate may beat any purchase. A model where Moves only buy and sell cannot express that, so it is
wrong.
