...
import { move, moveDirection } from '../src/db/schema/move.ts';
...
describe('the plan model', () => {
  ...
  it('directs moves at a destination rather than buying and selling', () => {
    expect(moveDirection.enumValues).toEqual(['into', 'out_of']);

    const columns = getTableConfig(move).columns.map((column) => column.name);
