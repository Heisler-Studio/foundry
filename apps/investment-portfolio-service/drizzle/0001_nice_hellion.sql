...
CREATE TYPE "public"."step_direction" AS ENUM('into', 'out_of');--> statement-breakpoint
CREATE TYPE "public"."step_status" AS ENUM('pending', 'done', 'skipped');--> statement-breakpoint
...
CREATE TABLE "step" (
	...
	"direction" "step_direction" NOT NULL,
	...
	CONSTRAINT "step_plan_sequence_unique" UNIQUE("plan_id","sequence"),
	CONSTRAINT "step_amount_positive" CHECK ("step"."amount" > 0),
	CONSTRAINT "step_no_self_dependency" CHECK ("step"."depends_on_step_id" is null or "step"."depends_on_step_id" <> "step"."id"),
	CONSTRAINT "step_executed_when_done" CHECK (("step"."status" = 'done') = ("step"."executed_at" is not null))
);
--> statement-breakpoint
...
ALTER TABLE "step" ADD CONSTRAINT "step_plan_id_plan_id_fk" FOREIGN KEY ("plan_id") REFERENCES "public"."plan"("id") ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
ALTER TABLE "step" ADD CONSTRAINT "step_destination_id_destination_id_fk" FOREIGN KEY ("destination_id") REFERENCES "public"."destination"("id") ON DELETE restrict ON UPDATE no action;--> statement-breakpoint
ALTER TABLE "step" ADD CONSTRAINT "step_depends_on_step_id_step_id_fk" FOREIGN KEY ("depends_on_step_id") REFERENCES "public"."step"("id") ON DELETE set null ON UPDATE no action;--> statement-breakpoint
